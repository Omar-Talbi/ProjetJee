...
public class EnseignantsDao {
    ...
    public List<Enseignant> getEnseignants() {
        try {
            ...
            while (rs.next()) {
                Enseignant ens = new Enseignant(rs.getString("id"), rs.getInt("cin"), rs.getString("prenom"), rs.getString("nom"), rs.getString("adresse"), rs.getString("email"), rs.getInt("phone"), rs.getString("login"),rs.getInt("authorized"));
                Grade g = new Grade();
    ...
    public Enseignant findEnsById(String id) {
        try {
            ...
            Enseignant ens;
            while (rs.next()) {
                ens = new Enseignant(rs.getString("id"), rs.getInt("cin"), rs.getString("prenom"), rs.getString("nom"), rs.getString("adresse"), rs.getString("email"), rs.getInt("phone"), rs.getString("login"),rs.getInt("authorized"));
                Grade g = new Grade();
                g.setId(rs.getInt("grade_id"));
                g.setLibelle(rs.getString("gradeLib"));
                Departement d = new Departement();
    ...
    public Enseignant updateEnseignant(Enseignant ens) {
        try {
            PreparedStatement pst = JdbcUtils.getConnection().prepareStatement("UPDATE `enseignant` SET cin = ?, prenom = ? , nom = ? , phone = ? , adresse = ? , departement_id = ? , grade_id = ? , authorized = ?  WHERE id = ? ;");
            ...
            pst.setInt(8, ens.getAuthorized());
            pst.setString(9, ens.getId());
            pst.execute();
            return ens;
        } catch (SQLException ex) {
            ex.printStackTrace();
